    ...
    .then(function(doc){    
        ...
        if(catV!==catN)// si el nombre de la categoria vieja es distinta de la seleccionada
        {
            ...
            ccat.get().then(function(doc){
                ...
                cat2.update({
                    Titulos:firebase.firestore.FieldValue.arrayUnion(info)
                });
                cat2.update({
        ...
        .then(function(doc){
            ...
            {   
                ...
                ccat.get().then(function(doc){
                    ...
                    var cat2=bd.collection('catalogo').doc(catN);
                    cat2.update({
                        Titulos:firebase.firestore.FieldValue.arrayUnion(nuevo)
                    });
                    cat2.update({
                        cantidad:firebase.firestore.FieldValue.increment(1)
                    });
    ...
    .then(function(doc){
        ...
        .then(function(querySnapshot)
        {
            ...
            .then(function(querySnapshot)
            {
                ...
                $$('#tituloEditar').append('<div class="card-header"><b>Titulo</b></div><div class="card-content card-content-padding"><div class="row"><input type="text" id="tituloE" class="colorFondo col-75" value="'+info+'" disabled/><div class="item-media"><a id="ediTit" class="col-10 visible" ><i class="fas fa-edit"></i></a></div></div><div class="row"><b>Categoria:</b><div class="item-input-wrap input-dropdown-wrap"><select id="idCat"></select></div></div></div>');                 
                ...
                {
                    var i=j+1;
                    $$('#pasosEditar').append('<div class="card" id="paso'+i+'"><div class="card-header"><b>Paso:</b></div><div class="card-content card-content-padding"> <div class="row"><textarea id="textoPaso'+i+'" value="'+arregloPasos[j]+'" class="colorFondo col-75" disabled>'+arregloPasos[j]+'</textarea><div class="oculto textoerror" id="errortextoenfermedad'+i+'" >Completar este campo.</div><div class="oculto textoerror" id="error2textoenfermedad'+i+'" >La cantidad de caracteres debe ser menor a 170.</div> <div class="item-media"><a href="#" id="edi'+i+'" class="editar col-10" ><i class="fas fa-edit"></i></a></div></div>	</div><div class="card-content card-content-padding row"><input type="file" class="audios oculto" name="audio'+i+'" id="audio'+i+'"><audio src="'+arregloAudios[j]+'" id="audrep'+i+'"></audio><button class="col-75 playEditar button button-fill visible" id="audbot'+i+'">Reproducir</button><div class="oculto textoerror" id="erroraudio'+i+'" >Completar este campo.</div><div class="oculto textoerror" id="error2audio'+i+'" >El archivo debe tener formato mp3.</div><div class="oculto textoerror" id="error3audio'+i+'" >El archivo debe durar menos de 30 segundos.</div><div class="item-media"><a href="#" id="audedi'+i+'" class="editarAudio col-10 visible" ><i class="fas fa-edit"></i></a> </div>	</div><div class="card-footer"><button class=" button button-small button-fill eliminarPaso" id="'+i+'">Eliminar</button></div></div>');                    
                }
                ...
                $$('#masPaso').on('click',function(){
                    $$('#pasosEditar').append('<div class="card" id="paso'+nroNuevoPasoE+'"><div class="card-header"><b>Paso:</b></div><div class="card-content card-content-padding"><textarea placeholder="Ingrese el texto del paso" id="textoPaso'+nroNuevoPasoE+'" class="colorFondo"></textarea><div class="oculto textoerror" id="errortextoenfermedad'+nroNuevoPasoE+'" >Completar este campo.</div><div class="oculto textoerror" id="error2textoenfermedad'+nroNuevoPasoE+'" >La cantidad de caracteres debe ser menor a 170.</div> </div><div class="card-content card-content-padding"><input type="file" class="audios nuevoPaso" name="audio'+nroNuevoPasoE+'" id="audio'+nroNuevoPasoE+'"  ><div class="oculto textoerror" id="erroraudio'+nroNuevoPasoE+'" >Completar este campo.</div><div class="oculto textoerror" id="error2audio'+nroNuevoPasoE+'" >El archivo debe tener formato mp3.</div><div class="oculto textoerror" id="error3audio'+nroNuevoPasoE+'" >El archivo debe durar menos de 30 segundos.</div></div><div class="card-footer"><button class=" button button-small button-fill eli eliminarPaso" id="'+nroNuevoPasoE+'">Eliminar</button></div></div>');
                    nroNuevoPasoE++;
...
{
    var arreglito=[];
    console.log('que ondis');
    var bd=firebase.firestore();
    bd.collection('catalogo').orderBy('Titulos').get()
    .then(function(querySnapshot){
        querySnapshot.forEach(function(doc){
...
function eliminarCategoria() /* NO SE CIERRA EL POPUP*/
{
    var bd=firebase.firestore();
